...
import 'package:nextgame_mobile/models/Platforma.dart';
import 'package:nextgame_mobile/models/Recenzija.dart';
import 'package:nextgame_mobile/models/Zanr.dart';
...
class _IgraDetaljiState extends State<IgraDetalji> {
  ...
  Widget build(BuildContext context) {
    Future<List<Recenzija>> GetRecenzija() async {
      ...
      var listaRecenzija =
          recenzija!.map((i) => Recenzija.fromJson(i)).toList();
      return listaRecenzija;
    }

    Future<List<Zanr>> GetZanrove(List<IgraZanr> zanroviIgrice) async {
      List<Zanr> zanrovi = <Zanr>[];
      for (var i = 0; i < zanroviIgrice.length; i++) {
        zanrovi.add(Zanr.fromJson(await APIService.GetById(
            'Zanr', zanroviIgrice[i].zanrId.toString())));
      }
      return zanrovi;
    }

    Future<List<Platforma>> GetPlatforme(
        List<IgraPlatforma> platformeIgrice) async {
      List<Platforma> platforme = <Platforma>[];
      for (var i = 0; i < platformeIgrice.length; i++) {
        platforme.add(Platforma.fromJson(await APIService.GetById(
            'Platforma', platformeIgrice[i].platformaId.toString())));
      }
      return platforme;
    }
    ...
    Future<void> UpdateRecenzija(stararecenzija, isPrijavljena) async {
      Recenzija novaRecenzija =
          new Recenzija(Id: stararecenzija.Id, isPrijavljena: isPrijavljena);
      var recenzija = await APIService.Update("Recenzija",
          jsonEncode(novaRecenzija.toJsonUpdate()), novaRecenzija.Id);
    }

    Widget platformaWidget(Platforma platforma) {
      return Text(platforma.naziv,
          style: TextStyle(fontSize: 18, color: Colors.white));
    }

    Widget platformaBuilder(platformeIgrice) {
      return FutureBuilder<List<Platforma>>(
        future: GetPlatforme(platformeIgrice),
        builder:
            (BuildContext context, AsyncSnapshot<List<Platforma>> snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              return Wrap(
                ...
                children:
                    snapshot.data!.map((e) => platformaWidget(e)).toList(),
              );
            }
          }
      );
    ...
    Widget zanroviWidget(Zanr zanr) {
      return Text(zanr.naziv,
          style: TextStyle(fontSize: 18, color: Colors.white));
    }

    Widget zanroviBuilder(zanroviIgrice) {
      return FutureBuilder<List<Zanr>>(
        future: GetZanrove(zanroviIgrice),
        builder: (BuildContext context, AsyncSnapshot<List<Zanr>> snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return SpinKitFadingFour(
              color: Colors.white,
              size: 50.0,
            ...
            if (snapshot.hasError) {
              ...
              return Wrap(
                direction: Axis.vertical,
                children: snapshot.data!.map((e) => zanroviWidget(e)).toList(),
              );
            }
          }
      );
              ...
              children: [
                    ...
                    children: [
                      CircleAvatar(
                          ...
                          backgroundImage: MemoryImage(
                              Uint8List.fromList(recenzija!.korisnik!.Slika))),
                      Text(
                ...
                Padding(
                  ...
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.start,
                    children: [
                      Text("Ocjena: " + recenzija!.ocjena.toString(),
                          style: TextStyle(fontWeight: FontWeight.bold)),
                      Padding(
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              return Wrap(
                direction: Axis.vertical,
                children:
                    snapshot.data!.map((e) => recenzijaWidget(e)).toList(),
              );
            }
          }
        },
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: Image(
                        image: MemoryImage(
                            Uint8List.fromList(widget.igrica!.slika))),
                  ),
                  Text(
                    widget.igrica!.naziv,
                    style: TextStyle(
                ...
                children: [
                  ...
                  Text(
                    widget.igrica!.ocjena == "0"
                        ? "Nema ocjene."
                        : widget.igrica!.ocjena.toString().length > 4
                            ? widget.igrica!.ocjena.toString().substring(0, 4) +
                                "/5"
                            : widget.igrica!.ocjena.toString() + "/5",
                    style: TextStyle(
                        fontSize: 20,
                        color: Colors.white,
                        fontWeight: FontWeight.bold),
                          ...
                          child: Align(
                            ...
                            child: Text(
                              widget.igrica!.datumIzdavanja!.day.toString() +
                                  "." +
                                  widget.igrica!.datumIzdavanja!.month
                                      .toString() +
                                  "." +
                                  widget.igrica!.datumIzdavanja!.year
                                      .toString(),
                              style: TextStyle(
                                fontSize: 16,
                                color: Colors.white,
                        ...
                        child: Column(
                          children: [
                            ...
                            Padding(
                              ...
                              child: Column(
                                children: [
                                  zanroviBuilder(widget.igrica!.zanrovi),
                                ],
                              ),
                            ),
                          ],
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Padding(
                        padding: const EdgeInsets.fromLTRB(80, 0, 0, 35),
                        child: Align(
                            alignment: Alignment.centerLeft,
                            child: Column(children: [
                              Padding(
                                padding: const EdgeInsets.only(left: 0),
                                child: Text("Platforme:",
                                    style: TextStyle(
                                      fontSize: 18,
                                      color: Colors.white,
                                      fontWeight: FontWeight.bold,
                              ...
                              platformaBuilder(widget.igrica!.platforme)
                            ])))
                  ],
                ),
              ),
